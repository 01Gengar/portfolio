...
class Car:
    ...
    def __init__(self, tank_size, gas_consumption, gas_in_tank=0, odometer_state=0):
        """
        ...
        :param gas_in_tank: float, how much gas is remaining in tank
        :param odometer_state: float, number on odometer
        """

        self.__tank_volume = tank_size
        self.__consumption = gas_consumption
        self.__gas = gas_in_tank
        self.__odometer = odometer_state

    def print_information(self):
        """
        Prints car fuel and odometer status.
        """
    ...
    def fill(self, gas_to_fill):
        ...
        :param gas_to_fill: How much gas does user want to fill.
        """
        if gas_to_fill < 0:
    ...
    def drive(self, distance):
        """
        Calculates how far the car has traveled for new state on odometer
        and how much fuel the car wasted in process.

        :param distance: float, Desired traveling distance.
        """
        if distance < 0:
            print("You cannot travel a negative distance")
        else:
            max_distance = self.__gas * 100 / self.__consumption
            if distance <= max_distance:
                needed_fuel = distance * self.__consumption / 100
                self.__odometer += distance
                self.__gas -= needed_fuel
            else:
